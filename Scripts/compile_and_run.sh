#!/usr/bin/env bash
# Reset EnterpriseAIMetrics: kill running instances, build, package, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ROOT_DIR}/EnterpriseAIMetrics.app"
APP_PROCESS_PATTERN="EnterpriseAIMetrics.app/Contents/MacOS/EnterpriseAIMetrics"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/EnterpriseAIMetrics"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/EnterpriseAIMetrics"
...
resolve_signing_mode() {
  ...
  for candidate in \
    ...
    "EnterpriseAIMetrics Development"
  do
...
kill_claude_probes() {
  # EnterpriseAIMetrics spawns `claude /usage` + `/status` in a PTY; if we kill the app mid-probe we can orphan them.
...
kill_all_codexbar() {
  is_running() {
    pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
      ...
      || pgrep -x "EnterpriseAIMetrics" >/dev/null 2>&1
  }
  ...
  for _ in {1..25}; do
    ...
    pkill -x "EnterpriseAIMetrics" 2>/dev/null || true
    if ! is_running; then
      return 0
    fi
    sleep 0.2
  ...
  pkill -9 -x "EnterpriseAIMetrics" 2>/dev/null || true
  ...
  fail "Failed to kill all EnterpriseAIMetrics instances."
}
...
# 2) Kill all running EnterpriseAIMetrics instances (debug, release, bundled).
log "==> Killing existing EnterpriseAIMetrics instances"
kill_all_codexbar
kill_claude_probes
...
if [[ "${SIGNING_MODE:-adhoc}" == "adhoc" && "${CLEAR_ADHOC_KEYCHAIN}" == "1" ]]; then
  log "==> Clearing EnterpriseAIMetrics keychain entries (adhoc signing)"
  ...
  log "==> Preserving EnterpriseAIMetrics keychain entries (pass --clear-adhoc-keychain to reset adhoc keychain state)"
fi
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/EnterpriseAIMetrics" >/dev/null 2>&1 &
  disown
fi
...
for _ in {1..10}; do
  if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
    log "OK: EnterpriseAIMetrics is running."
    exit 0
  fi
  sleep 0.4
done
