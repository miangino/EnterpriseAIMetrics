...
## Release flow
1) Build & notarize as usual (`./Scripts/sign-and-notarize.sh`), producing notarized `CodexBar-macos-universal-<ver>.zip` and a matching DMG.
...
3) Upload `appcast.xml`, zip, and DMG to GitHub Releases (feed URL stays stable).
4) Tag/release.
